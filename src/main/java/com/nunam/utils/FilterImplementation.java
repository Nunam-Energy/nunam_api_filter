...
public class FilterImplementation<T> {
	...
	public List<T> filter(Map<String, Object> filter, Integer offset, Integer limit, List<String> orderby, T to ) {
		...
		query.select(t).where(finalPredicate);
		List<Order> orderByPaths = new ArrayList<>();
		if (orderby != null && !orderby.isEmpty()) {
			for (String orderbyFiled : orderby) {
				Path<?> orderPath = t.get(orderbyFiled);
				orderByPaths.add(cb.desc(orderPath));
			}
			query.orderBy(orderByPaths);
